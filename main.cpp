    ...
    switch (opc)
    {
        ...
        case 4:
        {
            ...
            student.contains(number) ? 
                printMsg({ "There is a student that is associated to that number.\n" }, BoxStyle::Success) : 
                printMsg({ "There is no student that is associated to that number.\n" }, BoxStyle::Negative);
        ...
        {
            if (student.size() == 0) {
                printMsg({ "There are no elements in the tree.\n" }, BoxStyle::Warning);
                break;
            }
            ...
            for (const Student* s : students) {
                i++;
                printMsg({ "Student " + to_string(i) + ": " + to_string(s->number) + ", " + s->name + "\n" });
            }
        ...
        case 6:
        {
            ...
            student.remove(number) ?
                printMsg({ "Student successfully deleted.\n" }, BoxStyle::Success) :
                printMsg({ "There is no student associated to that number.\n" }, BoxStyle::Negative);
        ...
        default:
            printMsg({ "Select a number between 1-7.\n" }, BoxStyle::Warning);
            break;
    }
}
